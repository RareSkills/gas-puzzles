...
const logGasUsage = (currentGasUsage) => {
    ...
    if (diff < 0) {
        console.log(`           You are \x1b[31m${diff * -1}\x1b[0m below the target`);
    }
}
...
describe("Distribute", async function () {
    ...
    describe("Gas target", function () {
        it("The functions MUST meet the expected gas efficiency", async function () {
            ...
            expect(gasEstimate)
                .to.satisfy(function(val) { return val <= TARGET_GAS_PRICE });
        });
    });
    ...
    describe("Business logic", function () {
        it("The functions MUST perform as expected", async function () {
            ...
            await expect(instance.distribute()).to.be
                .rejectedWith('cannot distribute yet');
            ...
            expect(await ethers.provider.getBalance(acct1.address)).to
                .equal(new BigNumber.from(ethers.utils.parseEther('0.25')));
        });
    });
});
